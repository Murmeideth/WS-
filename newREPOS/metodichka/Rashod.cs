...
namespace metodichka
{
    public partial class Rashod : Form
    {
        ...
        {
            string query = "select items.name, rashod.quatity, rashod.price, rashod.income from items join rashod on prihod.id_tovar = items.Id_item order by outcome;";
        ...
        private void AddRashod_Click(object sender, EventArgs e)
        {
            string query = "insert into rashod values('" + Convert.ToString(comboBox1.SelectedIndex + 1) + "', '" + textBox1.Text + "', '" + textBox2.Text + "', '" + dateTimePicker1.Text + "');";
            MySqlConnection conn = DBUtils.GetDbConnection();
            MySqlCommand cmDB = new MySqlCommand(query, conn);
            cmDB.CommandTimeout = 60;
            try
