...
namespace Manager
{
    class functions
    {
        public void getQuests(string mode, string worker, ListView Quest_list)
        {
            if(mode == "admin")
            {
                string query = "select q.id_quest, q.desc, c.desc, e.name, if(q.status = 0, 'Выполняется', 'Выполнено') from quest_list as q, category as c, emplo as e where q.id_cat = c.id_cat and q.id_emplo_resp = e.id_emplo order by q.id_quest;";
            ...
            {
                string query = "select q.id_quest, q.desc, c.desc, e.name, if(q.status = 0, 'Выполняется', 'Выполнено') from quest_list as q, category as c, emplo as e where q.id_cat = c.id_cat and q.id_emplo_resp = e.id_emplo and e.name = '" + worker + "' order by q.id_quest;";
                MySqlConnection conn = DBUtils.GetDBConnection();
                MySqlCommand cmDB = new MySqlCommand(query, conn);
                cmDB.CommandTimeout = 60;
                MySqlDataReader rd;
        ...
        public void AddQuest(TextBox Desc, ListBox Category, ListBox Emplo)
        {
            int last_id;
            ...
            try
            {
                ...
                string query_add = "insert into quest_list values("+ (last_id + 1) + ", '" + Desc.Text + "', " + (Convert.ToInt16(Category.SelectedIndex) + 1) + ", " + (Convert.ToInt16(Emplo.SelectedIndex) + 1) + ", 0);";
        ...
        public void GetChengedQuest(TextBox Desc, ListBox Category, ListBox Emplo, TextBox ChangeID)
        {
        ...
        public void ChangeQuest(TextBox ch_id, TextBox Desc, ListBox Category, ListBox Emplo)
        {
            string query = "update quest_list as q set q.desc = '" + Desc.Text + "', q.id_cat = " + (Convert.ToInt16(Category.SelectedIndex) + 1) + ", q.id_emplo_resp = " + (Convert.ToInt16(Emplo.SelectedIndex) + 1) + " where q.id_quest = " + Convert.ToInt16(ch_id.Text) + ";";
            MySqlConnection conn = DBUtils.GetDBConnection();
            MySqlCommand cmDB = new MySqlCommand(query, conn);
            cmDB.CommandTimeout = 60;
            MySqlDataReader rd;
